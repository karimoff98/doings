...
Запись атомарная: сначала временный файл, потом переименование, предыдущая версия остаётся рядом как `database.json.bak`. Дальше в папке `backups/` лежат датированные копии: автоматические — не чаще раза в шесть часов и только если база изменилась, плюс отдельная копия перед импортом, очисткой, загрузкой примеров, обновлением схемы и восстановлением. Список копий, восстановление и удаление — в настройках (`⌘,`); если копию создать не удалось, опасное действие не выполняется без явного согласия. В браузерной версии данные хранятся в `localStorage`. Копию можно сохранить и загрузить из настроек (`⌘,`).
